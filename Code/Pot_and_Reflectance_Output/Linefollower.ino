...
//	else                              readData &= ~0x40;
	if ( analogRead(S2) > THRESHOLD2 ) newdata |= 0x20;
	else                              newdata &= ~0x20;
	if ( analogRead(S3) > THRESHOLD3 ) newdata |= 0x10;
	else                              newdata &= ~0x10;
	if ( analogRead(S4) > THRESHOLD4 ) newdata |= 0x08;
	else                              newdata &= ~0x08;
	if ( analogRead(S5) > THRESHOLD5 ) newdata |= 0x04;
	else                              newdata &= ~0x04;
	if ( analogRead(S6) > THRESHOLD6 ) newdata |= 0x02;
	else                              newdata &= ~0x02;
//	if ( analogRead(S7) > THRESHOLD7 ) newdata |= 0x01;
//	else                              newdata &= ~0x01;
